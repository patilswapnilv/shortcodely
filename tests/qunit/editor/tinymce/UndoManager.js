...
test(
    'No undo levels and one redo', function() {
        editor.undoManager.clear();
        editor.setContent('test');
...
test(
    'One undo levels and one redo', function() {
        editor.undoManager.clear();
        editor.setContent('test');
...
test(
    'Undo and add new level', function() {
        editor.undoManager.clear();
        editor.setContent('test');
...
test(
    'No undo/redo cmds on Undo/Redo shortcut', function() {
...
test(
    'Transact', function() {
        ...
        level = editor.undoManager.transact(
            function() {
...
test(
    'Transact no change', function() {
        editor.undoManager.add();

        var level = editor.undoManager.transact(
            function() {
            }
        );
...
test(
    'Exclude internal elements', function() {
        ...
        equal(count, 1);
        equal(
            Utils.cleanHtml(lastLevel.content),
            'test' +
            '<img src="about:blank">' +
...
test(
    'Undo added when typing and losing focus', function() {
        var lastLevel;
...
test(
    'Dirty state type letter', function() {
        editor.undoManager.clear();
...
test(
    'Dirty state type shift+letter', function() {
        editor.undoManager.clear();
        editor.setDirty(false);
        editor.setContent("<p>a</p>");
        Utils.setSelection('p', 1);
...
test(
    'Dirty state type AltGr+letter', function() {
        editor.undoManager.clear();
        editor.setDirty(false);
        editor.setContent("<p>a</p>");
        Utils.setSelection('p', 1);
