ModuleLoader.require(
    ...
    "tinymce/Env"
        ...
        asyncTest(
            'replace uploaded blob uri with result uri (copy/paste of an uploaded blob uri)', function() {
                editor.setContent(imageHtml(testBlobDataUri));
        ...
        asyncTest(
            'don\'t replace uploaded blob uri with result uri (copy/paste of an uploaded blob uri) since blob uris are retained', function() {
        ...
        asyncTest(
            'uploadImages (callback)', function() {
        ...
        asyncTest(
            'uploadImages retain blob urls after upload', function() {
                var uploadedBlobInfo;
                ...
                editor.uploadImages(assertResult).then(assertResult).then(
                    function() {
                        uploadedBlobInfo = null;

                        return editor.uploadImages(function() {}).then(
                            function(result) {
                                QUnit.strictEqual(result.length, 0);
                                QUnit.strictEqual(uploadedBlobInfo, null);
                            }
        ...
        asyncTest(
            'uploadConcurrentImages', function() {
        ...
        asyncTest(
            'uploadConcurrentImages (fail)', function() {
                var uploadCount = 0, callCount = 0;
        ...
        asyncTest(
            'Don\'t upload transparent image', function() {
                var uploadCount = 0;
        ...
        asyncTest(
            'Don\'t upload bogus image', function() {
                var uploadCount = 0;
        ...
        asyncTest(
            'Don\'t upload filtered image', function() {
                var uploadCount = 0;
        ...
        test(
            'Retain blobs not in blob cache', function() {
