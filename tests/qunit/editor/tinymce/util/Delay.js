ModuleLoader.require(
    ...
    "tinymce/util/Delay"
        ...
        test(
            'setEditorTimeout (removed)', function() {
        ...
        asyncTest(
            'setEditorInterval', function() {
                var count = 0, id, fakeEditor = {};

                id = Delay.setEditorInterval(
                    fakeEditor, function() {
                        if (++count == 2) {
                            Delay.clearInterval(id);
                            equal(count, 2);
        ...
        test(
            'setEditorInterval (removed)', function() {
                var fakeEditor = {removed: true};

                Delay.setEditorInterval(
                    fakeEditor, function() {
                        throw new Error("Still executing setEditorInterval.");
                    }
                );
