...
jQuery(
    function( $ ) {
            ...
            QUnit.test(
                'Initially, the update lock should be false', function( assert ) {
                    assert.strictEqual(wp.updates.ajaxLocked, false);
                }
            );

            QUnit.test(
                'The nonce should be set correctly', function( assert ) {
                    assert.equal(wp.updates.ajaxNonce, window._wpUpdatesSettings.ajax_nonce);
                }
            );

            QUnit.test(
                'decrementCount correctly decreases the update number', function( assert ) {
            ...
            QUnit.test(
                '`beforeunload` should only fire when locked', function( assert ) {
            ...
            QUnit.test(
                'Update lock is set when plugins are updating', function( assert ) {
            ...
            QUnit.test(
                'Plugins are queued when the lock is set', function( assert ) {
                    var value = [
                    {
            ...
            QUnit.test(
                'If plugins are installing (lock is set), the beforeUnload function should fire', function( assert ) {
                    wp.updates.updatePlugin(
                        {
                            plugin: 'test/test.php',
                            slug: 'test'
            ...
            QUnit.test(
                'Starting a plugin update should call the update API', function( assert ) {
                    wp.updates.updatePlugin(
                        {
                            plugin: 'test/test.php',
                            slug: 'test'
            ...
            QUnit.test(
                'Installing a plugin should call the API', function( assert ) {
            ...
            QUnit.test(
                'Deleting a plugin should call the API', function( assert ) {
            ...
            QUnit.test(
                'Update lock is set when themes are updating', function( assert ) {
            ...
            QUnit.test(
                'If themes are installing (lock is set), the beforeUnload function should fire', function( assert ) {
            ...
            QUnit.test(
                'Starting a theme update should call the update API', function( assert ) {
                    wp.updates.updateTheme({ slug: 'twentyeleven' });
                    assert.ok(jQuery.ajax.calledOnce);
                    assert.equal(jQuery.ajax.getCall(0).args[0].url, '/wp-admin/admin-ajax.php');
            ...
            QUnit.test(
                'Installing a theme should call the API', function( assert ) {
            ...
            QUnit.test(
                'Deleting a theme should call the API', function( assert ) {
