...
jQuery(window).load(
     ...
     */
            test(
                'generatePlaceholderAutoIncrementId generates unique IDs', function() {
            ...
            test(
                'it should parse _wpCustomizeMenusSettings.defaults into itself', function() {
                    deepEqual(window._wpCustomizeNavMenusSettings, api.Menus.data);
                } 
            );

            test(
                'empty menus should have no Menu Item Controls', function() {
                    ok(0 === wp.customize.Menus.getMenuControl(socialMenuId).getMenuItemControls().length, 'empty menus');
                } 
            );

            test(
                'populated menus should have no Menu Item Controls', function() {
            ...
            test(
                'there is a properly configured MenusPanel', function() {
                    var panel, sections;
            ...
            test(
                'there an expected MenuSection for the primary menu', function() {
            ...
            test(
                'changing a MenuNameControl change the corresponding menu value', function() {
                    var section, control;
            ...
            test(
                'manipulating a MenuItemControl works', function() {
            ...
            test(
                'api.Menus.getMenuControl() should return the expected control', function() {
            ...
            test(
                'api.Menus.getMenuItemControl() should return the expected control', function() {
